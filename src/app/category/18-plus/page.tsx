...
export default function EighteenPlusCategoryPage() {
  ...
  const AdBanner = () => (
    <div className="col-span-full my-4 p-6 bg-accent/20 rounded-lg text-center">
        <h3 className="font-bold text-accent-foreground text-2xl add-your-ad">
            <Link href="/post-ad">ඔබේ දැන්වීම මෙහි පළ කරන්න!</Link>
        </h3>
    </div>
  );

  const renderCategory = (category: (typeof subCategories18Plus)[0]) => {
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-12 main-with-banners">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
          {subCategories18Plus.slice(0, 4).map(renderCategory)}
          <AdBanner />
          {subCategories18Plus.slice(4, 8).map(renderCategory)}
          <AdBanner />
          {subCategories18Plus.slice(8, 12).map(renderCategory)}
          <AdBanner />
          {subCategories18Plus.slice(12).map(renderCategory)}
        </div>
